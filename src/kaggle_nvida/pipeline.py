...
def create_bootstrap_mixed_pack(
    output_dir: Path,
    count_per_family: int = 100,
    seed: int = 7,
) -> dict[str, int]:
    """Generate a broader starter pack across math, retrieval, and structured tasks."""
    output_dir.mkdir(parents=True, exist_ok=True)
    expanded = []

    math_examples = generate_math_dataset(count=count_per_family, seed=seed)
    grouped_math = {}
    for example in math_examples:
        expanded.append(example.to_dict())
        grouped_math.setdefault(example.problem_id, []).append(example)

    for variants in grouped_math.values():
        full_variant = next(example for example in variants if example.variant_id == "full")
        wrong_answer = str(int(full_variant.target.final_answer) + 1)
        expanded.append(build_candidate_selection_variant(full_variant, wrong_answer).to_dict())
        expanded.append(build_self_correction_variant(full_variant, wrong_answer).to_dict())

    for example in generate_document_reasoning_dataset(count=count_per_family, seed=seed + 11):
        expanded.append(example.to_dict())

    for example in generate_structured_instruction_dataset(count=count_per_family, seed=seed + 19):
        expanded.append(example.to_dict())
